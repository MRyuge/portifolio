...
begin
  ...
  if (b) then begin
    try
      ...
      xf.Close;
      MessageDlg('A extração ocorreu correctamente', mtInformation, [mbOK], 0);
    except
      MessageDlg('Houve um erro durante a extração dos arquivos.', mtError, [mbOK], 0);
    end;
  end;
end;
...
begin
  ...
  if (b) then begin
    ...
    MessageDlg('O arquivo não parece correto.', mtWarning, [mbOK], 0);
  end;
